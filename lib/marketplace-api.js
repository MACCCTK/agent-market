...
export async function searchCapabilities(searchRequest) {
  try {
    return await requestJson("/marketplace/search-capabilities", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        task_template_id: searchRequest.task_template_id,
        keyword: searchRequest.keyword || null,
        min_reputation_score: searchRequest.min_reputation_score || 0,
        required_tags: searchRequest.required_tags || null,
        required_tools: searchRequest.required_tools || null,
        top_n: searchRequest.top_n || 3
      })
    });
  } catch (error) {
    return {
